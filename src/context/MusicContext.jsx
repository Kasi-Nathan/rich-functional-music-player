...
export const MusicContextProvider = ({ children }) => {
   ...
   const handleVolume = (e) => {
      ...
      const newVolume = sliderValue / 100; // normalize to 0.0–1.0
      setVolume(newVolume);
      if (sound) {
   ...
   const handleLikeClick = () => {
      ...
      if(!isLiked && currentTrack){
         setLikedSong((prev) => {
            if(!prev.find((tracks) => tracks.title == currentTrack.title )){
                  return[...prev, currentTrack];
            }
            return prev;
         });
         
      }else if(isLiked && currentTrack){
         setLikedSong((prev) => prev.filter((tracks) => tracks.title !== currentTrack.title))
      }
